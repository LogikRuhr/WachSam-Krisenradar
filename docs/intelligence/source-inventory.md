...
| 15 | `src/extractors/llm_extractor.py` + `prompts.py` | LLM | RSS-Roh → strukturierter Draft | Vertex AI via `google-genai`, Default `gemini-3.5-flash` / `global`, Modell konfigurierbar | extrahiertes `IngestionItem` `status=extracted` | DB-Draft (nicht published) | ~ | J | J (`GOOGLE_CLOUD_PROJECT`, ADC oder `GOOGLE_APPLICATION_CREDENTIALS`, `GEMINI_MODEL_NAME`) | pro Lauf | J (19, gemockt) | aktiv | Schema-/Enum-Validierung vorhanden; ADC-Fallback vorhanden; Quota-Retry/Circuit-Breaker vorhanden; JSON-Fence-Stripping bleibt regex-basiert | Agent-Platform-/ADK-Pilot nur separat, keine Auto-Publish-Änderung |
